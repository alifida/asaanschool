...
class Usertype_Model extends CI_Model {
	...
	public function merge($data) {
		...
		if (isset($data['id']) && !empty($data['id'])) {
			...
			$this->db->update('user_types', $data); // update the record
			...
			$this->db->insert('user_types', $data); // insert new record
	...
	public function getByKeys($keys = null) {
		if (empty ( $keys )) {
			return array ();
		}
		
		$this->db->select ()->from ( 'user_types' );
		$this->db->where_in ( 'internal_key', $keys );
